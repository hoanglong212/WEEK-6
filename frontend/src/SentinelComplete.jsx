...
const RiskGauge = ({score}) => {
  ...
  return (<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10}}><div style={{position:"relative",width:148,height:148}}><svg width="148" height="148" style={{transform:"rotate(-90deg)",position:"absolute"}}><defs><linearGradient id="arc-grad" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor={stopA}/><stop offset="100%" stopColor={stopB}/></linearGradient></defs><circle cx="74" cy="74" r={R} fill="none" stroke="rgba(255,255,255,.05)" strokeWidth="9"/><circle cx="74" cy="74" r={R} fill="none" stroke="url(#arc-grad)" strokeWidth="9" strokeLinecap="round" strokeDasharray={C} strokeDashoffset={C-(pct*C)}/></svg><div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><div className="f-orb" style={{fontSize:32,fontWeight:900,color:stopA,lineHeight:1}}>{score}</div><div className="f-mono" style={{fontSize:8,color:stopA,letterSpacing:2,marginTop:4,opacity:.7}}>/100</div></div></div><div className="f-mono" style={{fontSize:9,letterSpacing:3,color:stopA,background:`${stopA}12`,border:`1px solid ${stopA}30`,padding:"3px 12px",borderRadius:4}}>{label}</div></div>);
};
const StatCard = ({icon,label,value,color,raw}) => (<div style={{background:`linear-gradient(135deg,${color}08,rgba(0,0,0,.4))`,border:`1px solid ${color}20`,borderRadius:12,padding:"18px 14px",textAlign:"center",position:"relative",overflow:"hidden"}}><div style={{fontSize:22,marginBottom:8}}>{icon}</div><div className="f-orb" style={{fontSize:24,color,fontWeight:900,lineHeight:1}}>{raw?value:value}</div><div className="f-mono" style={{fontSize:8,color:"rgba(100,140,170,.6)",letterSpacing:1.5,marginTop:6}}>{label}</div></div>);
...
const Counter = ({target,suffix="",label,color="#00FF41"}) => {
  ...
  return (<div style={{textAlign:"center",padding:"32px 16px",borderRight:"1px solid rgba(0,255,65,.06)"}}><div className="f-orb" style={{fontSize:"clamp(28px,3vw,44px)",fontWeight:900,color,lineHeight:1,letterSpacing:-1}}>{fmt(target)}{suffix}</div><div className="f-mono" style={{color:"rgba(200,220,238,.5)",fontSize:10,letterSpacing:2,marginTop:10,textTransform:"uppercase"}}>{label}</div></div>);
};
...
const Step=({num,title,desc,delay=0})=>(<div className="fade-in" style={{display:"flex",gap:24,alignItems:"flex-start",animationDelay:delay+"s"}}><div style={{flexShrink:0,width:52,height:52,borderRadius:"50%",border:"1px solid rgba(0,255,65,.3)",display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,255,65,.06)"}}><span className="f-orb" style={{color:"#00FF41",fontSize:16,fontWeight:700}}>{num}</span></div><div><div className="f-orb" style={{color:"#00FF41",fontSize:13,fontWeight:600,letterSpacing:1,marginBottom:8}}>{title}</div><div style={{color:"rgba(200,220,238,.6)",fontSize:14,lineHeight:1.7}}>{desc}</div></div></div>);
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:80,alignItems:"center",maxWidth:1100,margin:"0 auto"}}>
        ...
        <div className="fade-in" style={{position:"relative"}}>
          <div className="glass-lp" style={{borderRadius:12,padding:32,position:"relative",overflow:"hidden"}}>
            ...
            {[{label:"PHISHING LINK DETECTED",val:"BLOCKED",color:"#FF6060",time:"0.12ms"},{label:"SENDER VERIFIED",val:"CLEAN",color:"#00FF41",time:"0.08ms"},{label:"ATTACHMENT SANDBOXED",val:"SCANNING",color:"#FFD60A",time:"0.31ms"},{label:"HEADER SPOOFING",val:"BLOCKED",color:"#FF6060",time:"0.09ms"},{label:"URL REPUTATION CHECK",val:"CLEAN",color:"#00FF41",time:"0.15ms"},{label:"BEHAVIORAL ANALYSIS",val:"ANOMALY",color:"#FF9500",time:"0.22ms"}].map((item,i)=>(<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 0",borderBottom:"1px solid rgba(0,255,65,.06)"}}><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{width:5,height:5,borderRadius:"50%",background:item.color,flexShrink:0,boxShadow:`0 0 6px ${item.color}`}}/><span className="f-mono" style={{color:"rgba(200,220,238,.7)",fontSize:11}}>{item.label}</span></div><div style={{display:"flex",alignItems:"center",gap:12}}><span className="f-mono" style={{color:"rgba(200,220,238,.3)",fontSize:9}}>{item.time}</span><span className="f-mono" style={{color:item.color,fontSize:10,fontWeight:500,letterSpacing:1}}>{item.val}</span></div></div>))}
      ...
      <AnimatePresence>
        {phase!=="result"&&(
          <motion.section initial={{opacity:0,y:40}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-30,scale:.97}} transition={{delay:.6,duration:.6}}>
            <div className={`glass-card ${focused?"glass-card-glow":""}`} style={{padding:"28px 30px",marginBottom:20,transition:"all .3s"}}>
              ...
              <AnimatePresence mode="wait">
                {phase==="scanning"?(
                  ...
                  <motion.div key="btn" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                    ...
                    {error&&<p className="f-mono" style={{color:"#FF7A8C",fontSize:10,letterSpacing:.8,marginTop:16}}>{error}</p>}
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
